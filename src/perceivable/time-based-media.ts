...
export async function checkTimeBasedMedia(
  ...
  try {
    ...
    const issues = await page.evaluate(() => {
      ...
      document.querySelectorAll('audio:not([controls]), video:not([controls])').forEach(media => {
        findings.push({
            ...
            impact: "critical",
            suggestion: "Ensure media has accessible alternative text or transcripts.",
        });
        });
        ...
        document.querySelectorAll('video').forEach(video => {
            ...
            if (!hasAudioDesc) {
                findings.push({
                    ...
                    impact: "critical",
                    suggestion: 'Provide audio descriptions for visual content in videos.',
                });
            }
        });

        // 1.2.4 Captions (Live)
        document.querySelectorAll('video[live], audio[live]').forEach(liveMedia => {
        ...
        document.querySelectorAll('video').forEach(video => {
            ...
            if (!hasSignLangTrack) {
                findings.push({
                    ...
                    impact: "critical",
                    suggestion: 'Provide sign language interpretation for audio content in video.',
                });
            }
        });
        ...
        document.querySelectorAll('video').forEach(video => {
            const hasExtendedAudioDesc = video.querySelector('track[kind="descriptions"][extended]');
