...
export async function checkTextAlternatives(
  ...
  try {
    ...
    const issues = await page.evaluate(() => {
      ...
      // 2️b SVG Icons <svg>
      document.querySelectorAll('svg').forEach(svg => {
        if (!svg.hasAttribute('role') || svg.getAttribute('role')?.trim() === '') {
          findings.push({
                type: 'SVG Icon',
                element: 'svg',
                path: getElementPath(svg),
      ...
      document.querySelectorAll('input[type="image"]').forEach(input => {
          if (!input.hasAttribute('alt') || input.getAttribute('alt')?.trim() === '') {
            findings.push({
                  type: 'Image Button',
      ...
      document.querySelectorAll('area').forEach(area => {
          if (!area.hasAttribute('alt') || area.getAttribute('alt')?.trim() === '') {
            findings.push({
                  ...
                  description: `Image Map Area '${area.href || 'No Link'}' is missing alt text`,
                  wcagCriteria: "1.1.1 Non-text Content",
                  impact: "critical",
      ...
      document.querySelectorAll('object').forEach(object => {
          if (!object.hasAttribute('title') && !object.hasAttribute('aria-label')) {
            findings.push({
                  type: 'Embedded Object',
                  element: 'object',
                  path: getElementPath(object),
                  description: `Embedded Object '${object.getAttribute('data') || 'No Data'}' is missing alt text`,
                  wcagCriteria: "1.1.1 Non-text Content",
                  impact: "critical",
                  suggestion: 'Use the title attribute or aria-label for accessibility.',
              });
          }
      });
      ...
      document.querySelectorAll('iframe').forEach(iframe => {
          if (!iframe.hasAttribute('title') || iframe.getAttribute('title')?.trim() === '') {
            findings.push({
                  ...
                  description: `Iframe '${iframe.src || 'No Source'}' is missing title text`,
                  wcagCriteria: "1.1.1 Non-text Content",
                  impact: "critical",
      ...
      document.querySelectorAll('video').forEach(video => {
          if (!video.querySelector('track[kind="captions"]')) {
            findings.push({
                  ...
                  description: `Video '${video.getAttribute('src') || 'No Source'}' is missing track text`,
                  wcagCriteria: "1.1.1 Non-text Content",
                  impact: "critical",
      ...
      document.querySelectorAll('audio').forEach(audio => {
          if (!audio.querySelector('track[kind="descriptions"]')) {
            findings.push({
                  ...
                  description: `Audio '${audio.getAttribute('src') || 'No Source'}' is missing track text`,
                  wcagCriteria: "1.1.1 Non-text Content",
                  impact: "critical",
                  suggestion: 'Provide a <track> with kind="descriptions" for transcripts.',
              });
          }
      });
      ...
      document.querySelectorAll('canvas').forEach(canvas => {
          if (!canvas.hasAttribute('aria-label')) {
            findings.push({
                  ...
                  wcagCriteria: "1.1.1 Non-text Content",
                  impact: "critical",
                  suggestion: 'Use aria-label or provide fallback text inside <canvas>.',
              });
          }
      });
      ...
      document.querySelectorAll('a').forEach(link => {
          if (!link.textContent?.trim() && !link.hasAttribute('aria-label')) {
            findings.push({
                  ...
                  description: `Link with Icon '${link.href || 'No Link'}' is missing aria-label text`,
                  wcagCriteria: "1.1.1 Non-text Content",
                  impact: "critical",
                  suggestion: 'Add an aria-label to describe the purpose of the link.',
              });
          }
      });
      ...
      document.querySelectorAll('canvas, object, iframe, embed, video').forEach(element => {
          if (!element.hasAttribute('aria-label') && !element.hasAttribute('title')) {
            findings.push({
                  ...
                  wcagCriteria: "1.1.1 Non-text Content",
                  impact: "critical",
                  suggestion: 'Provide a text description using aria-label or title.',
              });
          }
      });

      // 1.5 Sensory Content
      document.querySelectorAll('video, audio, canvas, embed, object').forEach(element => {
          if (!element.hasAttribute('aria-describedby') && !element.hasAttribute('aria-label')) {
            findings.push({
                  type: 'Sensory Content',
                  element: element.tagName.toLowerCase(),
                  path: getElementPath(element),
                  description: 'Sensory Content is missing aria-describedby or aria-label text',
                  wcagCriteria: "1.1.1 Non-text Content",
                  impact: "critical",
                  suggestion: 'Provide captions, transcripts, or an ARIA description.',
              });
          }
      });

      // 1.6 CAPTCHA Content
      document.querySelectorAll('img, audio, object, embed, iframe, div, span').forEach(element => {
        ...
        const captchaKeywords = ["captcha", "challenge", "verification", "security check"];
        ...
        const knownCaptchaClasses = ["g-recaptcha", "h-captcha", "cf-turnstile"];
        const isKnownCaptcha = knownCaptchaClasses.some(keyword => 
        ...
        if (isKnownCaptcha || isCaptcha) {
            findings.push({
                ...
                element: element.tagName.toLowerCase(),
                path: getElementPath(element),
                description: 'CAPTCHA detected, ensure an accessible alternative is available.',
                wcagCriteria: "1.1.1 Non-text Content",
                impact: "critical",
                suggestion: 'Provide alternative verification methods like audio CAPTCHA, text-based CAPTCHA, or human assistance.',
            });
        }

        // Detect iframes commonly used for CAPTCHA (Google reCAPTCHA, hCaptcha, etc.)
        if (element.tagName.toLowerCase() === "iframe" && /recaptcha|hcaptcha|captcha|turnstile/i.test(src)) {
            findings.push({
                type: 'CAPTCHA',
                element: element.tagName.toLowerCase(),
                path: getElementPath(element),
                description: 'CAPTCHA iframe detected, ensure an accessible alternative is available.',
                wcagCriteria: "1.1.1 Non-text Content",
                impact: "critical",
                suggestion: 'Provide alternative verification methods like audio CAPTCHA, text-based CAPTCHA, or human assistance.',
            });
      ...
      document.querySelectorAll('img, svg, div, span, i, icon, canvas, iframe').forEach(element => {
        ...
        const isVisuallyHidden = htmlElement.style.display === "none" || 
                                  htmlElement.style.visibility === "hidden" || 
                                  htmlElement.offsetWidth === 0 || 
                                  htmlElement.offsetHeight === 0;

        // Decorative elements should have aria-hidden="true", role="presentation", or alt=""
        const isDecorative = (ariaHidden === "true" || role === "presentation" || (element.tagName.toLowerCase() === "img" && alt === ""));
        ...
        if (!isDecorative && isVisuallyHidden) {
            findings.push({
                ...
                element: element.tagName.toLowerCase(),
                path: getElementPath(element),
                description: 'Visually hidden content is missing proper accessibility attributes.',
                wcagCriteria: "1.1.1 Non-text Content",
                impact: "moderate",
                suggestion: 'Use aria-hidden="true", role="presentation", or alt="" for purely decorative elements.',
            });
        }

        // Detect empty iframes (commonly used for decorative/formatting purposes)
        if (element.tagName.toLowerCase() === "iframe" && !element.hasAttribute('title')) {
            findings.push({
                type: 'Decorative Content',
                element: "iframe",
                path: getElementPath(element),
                description: 'Iframe is missing a title attribute, which may cause accessibility issues.',
                wcagCriteria: "4.1.2 Name, Role, Value",
                impact: "moderate",
                suggestion: 'Provide a descriptive title for meaningful iframes, or use aria-hidden="true" for decorative ones.',
            });
        }
      });
